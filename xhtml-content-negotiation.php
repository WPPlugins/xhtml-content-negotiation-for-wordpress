...
Description: Sends content as application/xhtml+xml based on the browser's ACCEPT request.  Ignores Internet Explorer which can't handle application/xhtml+xml.
Version: 1.4
Author: Samir Shah
...
class xhtml_content_negotiation {
	// PHP4 constructor
	function xhtml_content_negotiation() {
		add_filter('option_html_type', array(&$this, 'set_content_type') );
	}
	
	function q_value($type, $accept_string) {
	...
	function set_content_type( $default ) {
		...
		// If the client is IE6 or greater, or pocket IE, stick to HTML
		if( preg_match('/(msie|mspie|pocket)/i', $_SERVER['HTTP_USER_AGENT']) 
